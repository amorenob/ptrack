...
# Add Test Targets to env var TARGETS
TARGETS='{"targets":[{"url":"https://www.exito.com/tecnologia/televisores","max_pages":1}]}'
DOCKER_ENV_FLAGS="$DOCKER_ENV_FLAGS -e TARGETS=$TARGETS"
...
docker run --rm -it \
    $DOCKER_ENV_FLAGS \
    priceco-scraper scrapy crawl html_saver \
